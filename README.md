...
├── assets/
│   ├── semiconductor_ai.png          # 預設大圖 1：封面 (AI 與晶圓整合)
│   ├── chamber_control.png           # 預設大圖 2：製程控制 (Chamber 即時監控)
│   ├── wafer_defect.png              # 預設大圖 3：缺陷診斷 (Wafer Map 掃描判讀)
│   ├── yield_stack.png               # 預設大圖 4：Agent 良率 (Agent Yield Stack)
│   ├── wafer_microscope.png          # 備份 5：晶圓顯微觀測 (Wafer Microscope)
│   ├── metrology_sensor.png          # 備份 6：雷射量測感測器 (Metrology Laser Sensor)
│   ├── litho_system.png              # 備份 7：EUV 光刻機光路系統 (EUV Lithography)
│   ├── fab_agv.png                   # 備份 8：黃光區無人搬運車 (Fab Cleanroom AGV)
│   ├── agent_network.png             # 備份 9：AI Agent 網路節點 (AI Agent Network)
│   ├── etching_plasma.png            # 備份 10：Chamber 電漿蝕刻 (Plasma Etching)
│   ├── rca_flow.png                  # 備份 11：自動化根因分析診斷流 (RCA Flowchart)
│   ├── hbm_stack.png                 # 備份 12：3D 封裝 HBM 堆疊 (HBM 3D Packaging)
│   ├── cleanroom_engineer.png        # 備份 13：無塵室穿兔裝工程師 (bunny suit Engineer)
│   ├── yield_graph.png               # 備份 14：良率攀升趨勢圖表 (Yield Trend Graph)
│   ├── diff.png                      # 📌 新增 15：擴散製程模組圖 (DIFF Module Process)
│   ├── imp.png                       # 📌 新增 16：離子植入製程模組圖 (IMP Module Process)
│   ├── tf_pvd.png                    # 📌 新增 17：物理氣相沉積薄膜模組圖 (TF PVD Module)
│   ├── tf_cvd.png                    # 📌 新增 18：化學氣相沉積薄膜模組圖 (TF CVD Module)
│   ├── litho.png                     # 📌 新增 19：黃光微影製程模組圖 (LITHO Module)
│   ├── dry_etch.png                  # 📌 新增 20：乾式蝕刻製程模組圖 (DRY ETCH Module)
│   ├── cmp.png                       # 📌 新增 21：化學機械平坦化模組圖 (CMP Module)
│   └── wet_etch.png                  # 📌 新增 22：濕式蝕刻與清洗模組圖 (WET ETCH Module)
