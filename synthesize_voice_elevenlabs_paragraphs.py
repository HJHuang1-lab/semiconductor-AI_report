...
# ──────────────────────────────────────────────────────────────────────
# 💡 ELEVENLABS 聲音複製 一鍵段落批量合成腳本
# ──────────────────────────────────────────────────────────────────────
# 這個腳本會把口播稿「分段」發送給 ElevenLabs 雲端 API，使用您複製的真實聲音，
# 生成 voice_slide_{slide_num}_{para_idx}.mp3 並儲存到 assets/ 目錄。
#
# 🛠️ 準備步驟：
# 1. 前往 https://elevenlabs.io/ 註冊一個帳戶。
# 2. 在 "Voices" -> "Instant Voice Cloning" 中上傳我們為您做好的 "語音_260531_153027.wav"。
# 3. 取得您的 xi-api-key (點選右上方個人檔案頁面 Profile -> API Key)。
# 4. 取得您的 Voice ID (在 Voice 詳情中複製 ID)。
...
if API_KEY == "您的_ELEVENLABS_API_KEY_填在這裡" or VOICE_ID == "您的_CLONED_VOICE_ID_填在這裡":
    print("❌ 錯誤：請先將您的 ElevenLabs API Key 與 Voice ID 填入腳本的第 27 與 28 行！")
    sys.exit(1)
...
print("🎙️ 正在使用您本人的 AI 克隆聲音逐段生成高音質導讀語音...")
...
for slide_num, paragraphs in slides_data.items():
    for para_idx, text in enumerate(paragraphs):
        total_tasks += 1
        print(f"\n📢 [Slide {slide_num} - 段落 {para_idx}] 正在發送合成請求 (字數: {len(text)} 字)...")
        ...
        try:
            ...
            if response.status_code == 200:
                out_path = os.path.join(assets_dir, f"voice_slide_{slide_num}_{para_idx}.mp3")
                with open(out_path, "wb") as f:
                    f.write(response.content)
                size_kb = os.path.getsize(out_path) / 1024.0
                print(f"✅ 成功！音訊已儲存於：voice_slide_{slide_num}_{para_idx}.mp3 ({size_kb:.1f} KB)")
                success_count += 1
            else:
        ...
        # Micro-sleep to respect API rate limits
        time.sleep(0.5)

print("\n" + "=" * 60)
print(f"🎉 所有 Slide 語音批量生成完畢！成功：{success_count}/{total_tasks}")
print("💡 提示：您現在可以直接在瀏覽器中開啟簡報播放器，網頁將自動完美播放您專屬的克隆聲音進行完美口播！")
print("=" * 60)
